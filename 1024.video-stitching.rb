# -*- coding: utf-8 -*-
#
# @lc app=leetcode id=5019 lang=ruby
#
...
# clips into segments that cover the entire sporting event ([0, T]).
...

# Greedy, given current interval [a, b], always choose the clip with
# biggest end point for all interval [x, y] such that a<=x<=b.

# @param {Integer[][]} clips
# @param {Integer} t
# @return {Integer}
def video_stitching(clips, t)
  clips.sort_by! { |a, b| [a, -b] }
  ans = 1
  x, ans = [0, 0], 0
  while !clips.empty?
    return -1 if clips[0][0] > x[1]
    max = x[1]
    while !clips.empty? && clips[0][0] <= x[1]
      max = [max, clips.shift[1]].max
    end
    if max != x[1]
      ans += 1
      x[1] = max
