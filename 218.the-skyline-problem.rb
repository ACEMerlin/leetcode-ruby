# -*- coding: utf-8 -*-
#
# @lc app=leetcode id=218 lang=ruby
#
...
# range [0, 10000]. The input list is already sorted in ascending
# order by the left x position Li. The output list must be sorted by
# the x position. There must be no consecutive horizontal lines of
# equal height in the output skyline. For instance, [...[2 3], [4 5],
# [7 5], [11 5], [12 7]...] is not acceptable; the three lines of
# height 5 should be merged into one in the final output as such:
# [...[2 3], [4 5], [12 7], ...]


# Divide and Conquer. The method to analyse the time complexity is the
# same as merge sort.
#
#   0. Solve the sub problem of 0 building: [].
#   1. Solve the sub problem of only 1 building: [[x1, y], [x2, 0]].
#   2. Solve the sub problem for the first n/2 buildings.
#      Solve the sub problem for the last n/2 buildings.
#      Combine these two sub skylines:
#      - Pick the smaller x coordinate of these two.
#      - Get the largest y at x.
#      - Update answer with [x, y] if y doesn't equal to the previous
#        height in answer.

# @param {Integer[][]} buildings
# @return {Integer[][]}
def get_skyline(buildings, lo=0, hi=buildings.size-1)
  return [] if lo > hi
  if lo == hi
    a, b, y = buildings[lo]
    return [[a, y], [b, 0]]
  ...
  m = (lo+hi)/2
  p = get_skyline(buildings, lo, m)
  q = get_skyline(buildings, m+1, hi)
  merge(p, q)
end

def merge(p, q, pH=0, qH=0, ans=[])
  if p.empty?
    q.each { |x| add_ans(x, ans) }
  elsif q.empty?
    p.each { |x| add_ans(x, ans) }
  else
    x1, x2 = p[0][0], q[0][0]
    if x1 < x2
      x, y = p.shift
      add_ans([x, [y, qH].max], ans)
      merge(p, q, y, qH, ans)
    elsif x1 > x2
      x, y = q.shift
      add_ans([x, [y, pH].max], ans)
      merge(p, q, pH, y, ans)
    else
      x, y1 = p.shift
      x, y2 = q.shift
      add_ans([x, [y1, y2].max], ans)
      merge(p, q, y1, y2, ans)
    end
  end
  ans
...
def add_ans(x, ans)
  ans << x if ans.empty? || ans[-1][1] != x[1]
end
